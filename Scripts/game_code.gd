...
func _ready() -> void:
	...
	duplicate_area = get_node("/root/stall/duplicate_area")

	# Get the cooking area node
	cooking_area = get_node("/root/stall/cooking_area")

	# Get the finished products area node
	finished_products_area = get_node("/root/stall/finished_products_area")
